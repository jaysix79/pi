...
if [ ! -d "/xxxx" ]; then
	...
	case $input in
	    [yY][eE][sS]|[yY])
			...
			chmod +x /home/pi/programs/command/PIpoweroff						| tee -a 	"$LOG_FILE"
			cp -a /home/pi/programs/command/PIpoweroff /usr/bin/PIpoweroff 				| tee -a 	"$LOG_FILE"
			chmod +x /home/pi/programs/command/PIreboot						| tee -a 	"$LOG_FILE"
			cp -a /home/pi/programs/command/PIreboot /usr/bin/PIreboot 				| tee -a 	"$LOG_FILE"
			sleep 5
			clear
